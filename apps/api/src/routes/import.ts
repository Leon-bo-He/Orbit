  ...
  app.post('/api/import', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const idea of exportedIdeas) {
        const mappedWorkspaceId = idea.workspaceId ? (wsIdMap.get(idea.workspaceId) ?? null) : null;
        const mappedConvertedTo = idea.convertedTo ? (contentIdMap.get(idea.convertedTo) ?? null) : null;

        await tx.insert(ideas).values({
          userId,
          workspaceId: mappedWorkspaceId,
          convertedTo: mappedConvertedTo,
          title: idea.title,
          note: idea.note ?? null,
    ...
    return reply.send({
      ok: true,
      imported: { workspaces: workspacesImported, contents: contentsImported, ideas: ideasImported },
    });
  });
};
...
interface WsExport {
  ...
  publishGoal?: unknown;
  stageConfig?: unknown;
  planTemplates?: TplExport[];
  contents?: ContentExport[];
}
...
interface ContentExport {
  ...
  plan?: PlanExport | null;
  references?: RefExport[];
  publications?: PubExport[];
}
